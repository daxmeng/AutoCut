...
def get_lists_by_txt(txt_file: str):
    with open(txt_file, "r", encoding="utf-8") as f:
        subs = f.read().splitlines()
    # 过滤空行（可选）
    subs = [line.strip() for line in subs if line.strip()]
    return subs
    
def generate_precise_srt(txt_file: str, audio_path: str, out_path: str):
    subs = get_lists_by_txt(txt_file)
    for index,item in enumerate(subs):
        srt_lines = []
        start_time = 0
        end_time = get_media_duration(f"{audio_path}ori_{index + 1}.wav")

        # SRT 格式：序号、时间轴、文本、空行
        srt_lines.append("1")
        srt_lines.append(f"{_format_srt_time(start_time)} --> {_format_srt_time(end_time)}")
        srt_lines.append(item)
        srt_lines.append("")
        # 写入 SRT 文件
        with open(f"{out_path}sub_{index + 1}.srt", "w", encoding="utf-8") as f:
            f.write("\n".join(srt_lines))
...
""" 综合媒体 """
def _extract_audio_from_video(video, output):
    cmd = [
        "ffmpeg",
        "-y",
        "-loglevel", "warning",
...
def get_media_duration(media_path):
    ...
    return int(float(result.stdout.strip())* 100) / 100

def add_text_for_video(in_path, out_path, text="群聊或私信获取台账系统资料", last_sec = 3):
    duration = get_media_duration(in_path)
    # 最后 last_sec = 3 秒的开始时间
    text_start = max(0, duration - last_sec)
...
def extract_audio_srt(original_video, original_audio, output_srt):
    ...
    _extract_audio_from_video(original_video, original_audio)
    _generate_srt(original_audio, output_srt, model=model)

def burn_subtitle_2_video(
        video_in,
        srt_file,
        video_out
):
    ...
    delete_file(ass_file)

def batch_burn_subtitle(
    txt_file,
    target_path
):
    subs = get_lists_by_txt(txt_file)
    for index,item in enumerate(subs):
        video_in = f"{target_path}video_{index + 1}.mp4"
        srt_file = f"{target_path}sub_{index + 1}.srt"
        video_out = f"{target_path}video_final_{index + 1}.mp4"
        burn_subtitle_2_video(video_in, srt_file, video_out)